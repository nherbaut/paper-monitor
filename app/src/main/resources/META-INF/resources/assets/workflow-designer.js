(() => {
    ...
    const yamlFromWorkflow = (workflow) => {
        ...
        workflow.states.forEach((state) => {
            ...
            if (requires.exclusionCriterion || requires.inclusionCriteria || requires.exclusionNotes || requires.inclusionNotes) {
                line(lines, 2, "requires:");
                if (requires.exclusionCriterion) { line(lines, 3, "exclusion_criterion:"); line(lines, 4, "taxonomy: " + requires.exclusionCriterion.taxonomy); line(lines, 4, "exactly: " + (requires.exclusionCriterion.exactly || 1)); }
    ...
    document.querySelectorAll("[data-workflow-designer]").forEach((designer) => {
        ...
        ], layout: { name: "preset" }, minZoom: 0.2, maxZoom: 3, wheelSensitivity: 0.2 });
        const renderInspector = () => { const state = stateById(selectedStateId); inspector.hidden = !state; if (!state) return; const requires = state.requires || {}; const criterion = requires.exclusionCriterion || requires.inclusionCriteria; designer.querySelector("[data-workflow-inspector-title]").textContent = state.label || state.id; fields.label.value = state.label || ""; fields.group.value = state.group || ""; fields.terminal.checked = !!state.terminal; fields.requirementKind.value = requires.exclusionCriterion ? "exclusion" : requires.inclusionCriteria ? "inclusion" : ""; fields.requirementTaxonomy.value = criterion?.taxonomy || ""; fields.requirementCount.value = criterion?.exactly || criterion?.min || 1; fields.requirementNotes.checked = !!(requires.exclusionNotes || requires.inclusionNotes); fields.reportBucket.value = state.report?.prismaBucket || ""; };
        ...
        const updateState = () => { const state = stateById(selectedStateId); if (!state) return; const oldId = state.id, label = fields.label.value.trim() || "Untitled state", group = nodeId(fields.group.value) || null, base = stateId(label, group); let nextId = base, suffix = 2; while (nextId !== oldId && stateById(nextId)) nextId = base + "_" + suffix++; if (nextId !== oldId) { state.id = nextId; workflow.transitions.forEach((transition) => { if (transition.from === oldId) transition.from = nextId; transition.to = transition.to.map((target) => target === oldId ? nextId : target); }); if (workflow.initialState === oldId) workflow.initialState = nextId; workflow.layout.nodes[nextId] = workflow.layout.nodes[oldId]; delete workflow.layout.nodes[oldId]; selectedStateId = nextId; } state.label = label; state.group = group; state.terminal = fields.terminal.checked; const taxonomy = nodeId(fields.requirementTaxonomy.value), count = Math.max(1, Number.parseInt(fields.requirementCount.value || "1", 10) || 1); state.requires = {}; if (fields.requirementKind.value === "exclusion") state.requires.exclusionCriterion = { taxonomy: taxonomy || "EXCLUSION", exactly: count }; if (fields.requirementKind.value === "inclusion") state.requires.inclusionCriteria = { taxonomy: taxonomy || "INCLUSION", min: count }; if (fields.requirementNotes.checked && fields.requirementKind.value) state.requires[fields.requirementKind.value + "Notes"] = "optional"; state.report = fields.reportBucket.value.trim() ? { prismaBucket: nodeId(fields.reportBucket.value) } : {}; render(); };
        ...
        Object.values(fields).forEach((field) => field?.addEventListener(field.type === "checkbox" || field.tagName === "SELECT" ? "change" : "input", updateState));
